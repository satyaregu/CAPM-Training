...
async function createSessionByAuthCode(req, cb) {
  try {
    ...
    await exports.createSession(req, uaaOptions, xsuaaToken);
    cb();
  } catch (err) {
    cb(err);
  }
...
async function createSession(req, uaaOptions, xsuaaToken) {
  let urlTenant = passportUtils.getUrlTenant(req);
  let refreshToken = xsuaaToken.refresh_token;

  // Failover: if refresh token is not present in xsuaaToken, fetch it using the old flow
  if (!refreshToken) {
    tracer.warning('Refresh token not found in XSUAA jwt-bearer response, falling back to deprecated user_token grant flow');
    try {
      const refreshTokenResponse = await tokenUtils.getRefreshToken(xsuaaToken.access_token, uaaOptions);
